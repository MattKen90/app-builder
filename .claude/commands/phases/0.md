...
```
1. Read `.mode` file
2. If mode != "enhancer" → Error: "Phase 0 is for enhancer mode only. Run /mode enhancer first."
...
Write all outputs to `.enhancer/`:
```
.enhancer/
├── DISCOVERY.md     ← Your primary output
└── state/
    └── project.json ← Update with Phase 0 completion
```
